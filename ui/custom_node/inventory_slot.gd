...
func _can_drop_data(at_position: Vector2, original_slot: Variant) -> bool:
	return original_slot is InventorySlot
	
	
func _drop_data(at_position: Vector2,original_slot: Variant) -> void:
	EventSystem.INV_switch_two_item_indexes.emit(original_slot.get_index(),get_index())
